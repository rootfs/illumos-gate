...
/**
 * \brief Global data structures and functions for the dbuf cache.
 */
...
/**
 * \brief dbuf hash table routines
 */
static dbuf_hash_table_t dbuf_hash_table = { 0 };
...
/**
 * \brief Evict the dbuf's user, either immediately, or use a provided queue.
 *
...
/**
 * \brief Replace the current user of the dbuf.
 *
 ...
 *
 * \returns the old user, which may not necessarily be +old_user+.
 */
dmu_buf_user_t *
...
/**
 * \brief Set the user eviction data for the DMU buffer.
 *
 * \param db_fake		The DMU buffer to set the data for.
 * \param user			The user's eviction data pointer.
 *
 * \returns NULL on success, or the existing user pointer if it
 *	    is already set.
 */
dmu_buf_user_t *
...
/**
 * \brief Remove the user eviction data for the DMU buffer.
 *
...
/**
 * \brief Loan out an arc_buf for read.
 *
...
/**
 * \brief Shortcut for performing reads on bonus dbufs.
 *
...
/**
 * \brief Handle reads on dbufs that are holes, if necessary.
 *
...
dbuf_read(dmu_buf_impl_t *db, zio_t *zio, uint32_t flags)
{
	...
	if (db->db_state == DB_CACHED) {
		...
		/*
		 * Another reader came in while the dbuf is in flight
		...
		/* Skip the wait on the caller's request. */
...
/**
 * \brief Signal that the dirty record is about to be re-dirtied after sync.
 *
 * \param dr	The dirty record to update.
 *
    ...
    dmu_tx_t *tx, list_t *evict_list_p)
{
	...
	tmpprintf("%s db %p dr %p holds %lu dirties %d txg %"PRIu64"\n",
	...
	if (old_txg_is_frontend) {
		if (newest == db->db_data_pending) {
			...
			ASSERT(db->db_state & (DB_READ|DB_PARTIAL) ||
			    newest->dt.dl.dr_override_state == DR_OVERRIDDEN);
...
/**
 * \brief Indicate that you are going to modify the buffer's data (db_data).
 *
...
/**
 * \brief Try to directly assign a specific arcbuf to the provided dbuf
 *
 ...
 * Unfortunately, when we are not holding the dn_dbufs_mtx, we can't clear the
 * entry in the dn_dbufs list.  We have to wait until dbuf_destroy() in this
 * case.  For callers from the DMU we will usually see:
 * <tt>dbuf_clear()->arc_buf_evict()->dbuf_do_evict()->dbuf_destroy()</tt> For
 * the arc callback, we will usually see:
 * <tt>dbuf_do_evict()->dbuf_clear();dbuf_destroy()</tt> Sometimes, though, we
 * will get a mix of these two: DMU: <tt>dbuf_clear()->arc_buf_evict()</tt>
...
/**
 * \brief Returns with db_holds incremented, and db_mtx not held.
 *
 * \note buf_set may be NULL.
 * \note dn_struct_rwlock must be held.
 */
int
...
/**
 * \brief dbuf_rele() for an already-locked dbuf.
 *
 * This is necessary to allow db_dirtycnt and db_holds to be updated
 * atomically.
 */
void
...
/**
 * \brief Tells if the given dbuf is freeable.
 */
boolean_t
...
/**
 * \brief Sync an indirect block to disk.
 *
 */
static void
...
/**
 * \brief Commit a dirty buffer to disk.
 */
static zio_t *
dbuf_write(dbuf_dirty_record_t *dr, arc_buf_t *data, dmu_tx_t *tx)
{
	...
	if (parent != dn->dn_dbuf) {
		...
		/* We have a dirty parent and has been scheduled for write. */
		ASSERT(parent && parent->db_data_pending);
		/* Our parent's buffer is one level closer to the dnode. */
		ASSERT(db->db_level == parent->db_level-1);
		/* Nobody can find the old parent in the ARC. */
