    ...
    sa_handle_type_t hdl_type, sa_handle_t **handlepp)
{
	...
	if (handle == NULL) {
		...
		bzero(&handle->sa_dbu, sizeof(dmu_buf_user_t));
...
{
	dmu_buf_user_t *new_user = &newhdl->sa_dbu;
	dmu_buf_user_t *old_user = &oldhdl->sa_dbu;

	dmu_buf_init_user(new_user, sa_evict, NULL);
