...
/**
 * \file vdev_cache.c
 * \brief Virtual device read-ahead caching.
 *
 ...
 * vdev's vdev_cache.
 *
 * \todo Note that with the current ZFS code, it turns out that the
...
/**
 * \brief Evict the specified entry from the cache.
 */
static void
...
/**
 * \brief  Allocate an entry in the cache.  At the point we don't have the data,
 * we're just creating a placeholder so that multiple threads don't all go off
 * and read the same blocks.
 */
...
/**
 * \brief Fill a previously allocated cache entry with data.
 */
static void
 ...
 *
 * Returns 0 on cache hit, errno on a miss.
 */
int
...
/**
 * \brief Update cache contents upon write completion.
 */
void
