...
#define	DS_FIND_SELF		(0)
#define	DS_FIND_SNAPSHOTS	(1<<0)
#define	DS_FIND_CHILDREN	(1<<1)
#define	DS_FIND_IGNORE_SELF	(1<<2)
#define	DS_FIND_ALL		(DS_FIND_SNAPSHOTS|DS_FIND_CHILDREN)
#define	DS_FIND_ALL_BUT_SELF	(DS_FIND_ALL|DS_FIND_IGNORE_SELF)
...
int dmu_objset_clone(const char *name, struct objset *clone_origin,
    uint64_t flags);
...
/**
 * These structures are for DMU consumers that want async
 * callbacks.
 */
struct dmu_context;
struct dmu_buf_set;
 ...
 *
 * \retval 0       Success
 * \retval EBUSY   There are holds on this object's buffers
 *                 (via dmu_buf_hold()) or tx holds on the object
 *                 (via dmu-tx_hold_object()) so it cannot be freed
 ...
 * 				since txt
 *
 * \invariant  Can only be called on a objset with no dirty data.
 *
 * \retval 0       Success
...
/**
 * \return  The user object set with dmu_buf_set_user(), or NULL if not set.
 */
 ...
 *
 * \invariant  The data associated with the dbuf being committed to the
 *             intent log mux be stable while dmu_sync() is writing it.
 *
 * \retval EEXIST	This txg has already been synced, so there's nothing
 *			to to.  The caller should not log the write.
 * \retval ENOENT	The block was dbuf_free_range()'d, so there's nothing
 *			to do.  The caller should not log the write.
 * \retval EALREADY	This block is already in the process of being synced.
 *			The caller should track its progress (somehow).
 * \retval EIO		Could not do the I/O.  The caller should do a
 *			txg_wait_synced().
 * \retval 0		The I/O has been initiated.  The caller should log
 *			this blkptr in the done callback.  It is possible that
 *			the I/O will fail, in which case the error will be
 *			reported to the done callback and propagated to pio
 *			from zio_done().
 */
