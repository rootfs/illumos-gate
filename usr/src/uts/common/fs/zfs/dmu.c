...
/*
 * Error reporting for buf_set and dmu_context objects.  These share a
 * mutex because they are not expected to happen frequently, so they should
 * only be called if an error occurs.
 */
static void
dmu_buf_set_set_error(dmu_buf_set_t *buf_set, int err)
{
	mutex_enter(&buf_set->dmu_ctx->mtx);
	buf_set->err = zio_worst_error(buf_set->err, err);
	mutex_exit(&buf_set->dmu_ctx->mtx);
}

static void
dmu_context_set_error(dmu_context_t *dmu_ctx, int err)
{
	mutex_enter(&dmu_ctx->mtx);
	dmu_ctx->err = zio_worst_error(dmu_ctx->err, err);
	mutex_exit(&dmu_ctx->mtx);
}
...
static void
dmu_buf_do_uio(dmu_buf_set_t *buf_set, dmu_buf_t *db, uint64_t off,
    uint64_t sz, enum uio_rw dir)
{
	...
	if (err)
		dmu_buf_set_set_error(buf_set, err);
#endif
}
...
dmu_buf_read_uio(dmu_buf_set_t *buf_set, dmu_buf_t *db, uint64_t off,
    uint64_t sz)
{
	dmu_buf_do_uio(buf_set, db, off, sz, UIO_READ);
}
static void
dmu_buf_write_uio(dmu_buf_set_t *buf_set, dmu_buf_t *db, uint64_t off,
    uint64_t sz)
{
	dmu_buf_do_uio(buf_set, db, off, sz, UIO_WRITE);
}
 ...
 * NB: This can only be called once per buf_set, so accessing the buf_set's
 *     elements doesn't need a lock.
 */
static void
dmu_buf_set_complete(dmu_buf_set_t *buf_set)
{
	...
	/* Check again in case the transfer causes errors. */
	if (buf_set->err)
		dmu_context_set_error(dmu_ctx, buf_set->err);
...
dmu_thread_context_create(void)
{
	...
	return (tsd_set(zfs_async_io_key, dcs));
}
...
dmu_thread_context_destroy(void *context __unused)
{
	...
	VERIFY0(tsd_set(zfs_async_io_key, NULL));
}
 ...
 */
void
dmu_buf_set_rele(dmu_buf_set_t *buf_set, int err)
{
	dmu_context_t *dmu_ctx = buf_set->dmu_ctx;
...
dmu_issue(dmu_context_t *dmu_ctx)
{
	...
	while (dmu_ctx->resid > 0 && err == 0) {
		err = dmu_buf_set_init(dmu_ctx, &buf_set,
		    MIN(dmu_ctx->resid, DMU_MAX_ACCESS/2));
	...
	if (err == 0 && dmu_ctx->err != 0)
		...
		dmu_context_set_error(dmu_ctx, err);
    ...
    uint32_t flags)
{
	...
	mutex_init(&dmu_ctx->mtx, "context lock", MUTEX_DEFAULT, NULL);
	dmu_ctx->dn = dn;
	dmu_ctx->os = os;
