...
/**
 * \brief Obtain the DMU buffer from the specified object which contains the
 * specified offset.
 *
 * dmu_buf_hold() puts a "hold" on the buffer, so that it will remain in
 * memory.  You must release the hold with dmu_buf_rele().  You musn't access
 * the dmu_buf_t after releasing your hold.  You must have a hold on any
 * dmu_buf_t* you pass to the DMU.
 *
...
/**
 * \brief Asynchronously try to read in the data.
 */
void
...
/**
 * \brief Get the next "chunk" of file data to free.
 * 
 * We traverse the file from the end so that the file gets shorter over time
 * (if we crashes in the middle, this will leave us in a better state).  We
 * find allocated file data by simply searching the allocated level 1
 * indirects.
 */
static int
...
/**
 * \brief Free up the data blocks for a defined range of a file.
 *
 * If size is zero, the range from offset to end-of-file is freed.
 */
int
...
/**
 * \brief Handle a completed buffer set, and its DMU context if necessary.
 *
...
/**
 * \brief Allocate a loaned anonymous arc buffer.
 */
arc_buf_t *
...
/**
 * \brief Free a loaned arc buffer.
 */
void
...
/**
 * \brief When possible directly assign passed loaned arc buffer to a dbuf.
 *
...
/**
 * \brief Intent log support: sync the block associated with db to disk.
 *
 ...
 *
 *	\retval EEXIST this txg has already been synced, so there's nothing to to.
 *		The caller should not log the write.
 *	\retval ENOENT the block was dbuf_free_range()'d, so there's nothing to do.
 *		The caller should not log the write.
 *	\retval EALREADY this block is already in the process of being synced.
 *		The caller should track its progress (somehow).
 *	\retval EIO could not do the I/O.
 *		The caller should do a txg_wait_synced().
 *	\retval 0 the I/O has been initiated.
...
/**
 * \brief Set the data blocksize for an object.
 *
 ...
 *
 * \retval 0 Success
 * \retval EBUSY There are holds on the object contents
 * \retval ENOTSUP as described above
 */
int
...
/**
 * \brief Set the checksum property on a dnode.
 *
 * The new checksum algorithm will apply to all newly written blocks; existing
...
/**
 * \brief Set the compress property on a dnode.
 *
 * The new compression algorithm will apply to all newly written blocks;
 * existing blocks will not be affected.
 */
void
...
/**
 * \brief Decide how to write a block
 *
 * Set the policy for checksum, compression, number of copies, etc.
 */
void
...
/**
 * \brief Find the next hole or data block in file starting at *off
 *
...
/**
 * \brief Get information on a DMU object.
 *
 * If doi is NULL, just indicates whether the object exists.
 *
 * \retval 0 Success
 * \retval ENOENT Object is not allocated.
 */
void
...
/**
 * \brief Like dmu_object_info, but faster.
 *
...
/**
 * \brief Like dmu_object_info_from_db, but faster still.
 *
...
/**
 * \brief Initial setup and final teardown.
 */
void
dmu_init(void)
{
