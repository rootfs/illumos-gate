...
ztest_info_t ztest_info[] = {
	...
	ZI(ztest_reguid,			1,	&zopt_sometimes	),
	ZI(ztest_spa_rename,			1,	&zopt_rarely	),
	ZI(ztest_scrub,				1,	&zopt_rarely	),
	ZI(ztest_spa_upgrade,			1,	&zopt_rarely	),
	ZI(ztest_dsl_dataset_promote_busy,	1,	&zopt_rarely	),
	ZI(ztest_vdev_attach_detach,		1,	&zopt_sometimes	),
...
ztest_replay_truncate(ztest_ds_t *zd, lr_truncate_t *lr, boolean_t byteswap)
{
	...
	if (ztest_opts.zo_verbose >= 7)
		printf("%s: freeing obj %llu offset 0x%lx length 0x%lx "
...
ztest_dmu_read_write_zcopy(ztest_ds_t *zd, uint64_t id)
{
	...
	for (i = 0; i < 7; i++) {
		 ...
		 */
		if (ztest_opts.zo_verbose >= 7) {
			(void) printf("writing obj %llu offset %llx "
			    "size %llx txg %llx\n", packobj,
			    (u_longlong_t)bigoff,
			    (u_longlong_t)bigsize,
			    (u_longlong_t)txg);
...
ztest_commit_callback(void *arg, int error)
{
	...
	if (data->zcd_txg > synced_txg)
		fatal(0, "commit callback of txg %llu called prematurely"
		    ", last synced txg = %llu\n", data->zcd_txg,
		    synced_txg);
...
ztest_dmu_commit_callbacks(ztest_ds_t *zd, uint64_t id)
{
	...
	if (old_txg > txg)
		fatal(0, "future leak: got %llu, open txg is %llu",
		    old_txg, txg);
	...
	if (tmp_cb != NULL &&
		...
		fatal(0, "Commit callback threshold exceeded, oldest txg: %llu,"
		    " open txg: %llu\n", tmp_cb->zcd_txg, txg);
	}
